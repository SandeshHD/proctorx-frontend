...
export class ViewTestsComponent {
  ...
  getTests(first:number,rows:number){
    this.viewTestsService.getTests(first,rows,this.selectedBranch,this.show,this.query).subscribe(response=>{
      this.tests = response
  ...
  getBranches(){
    this.viewTestsService.getBranches().subscribe(response =>{
